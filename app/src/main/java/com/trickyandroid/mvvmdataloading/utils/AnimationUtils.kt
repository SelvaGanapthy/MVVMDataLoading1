...
object AnimationUtils {
    ...
    fun animateSunblind(holder: RecyclerView.ViewHolder, goesDown: Boolean) {
        ...
        holder.itemView.pivotY = (if (goesDown == true) 0 else holderHeight).toFloat()
        holder.itemView.pivotX = holder.itemView.height.toFloat()
        val animatorSet = AnimatorSet()
        val animatorTranslateY = ObjectAnimator.ofFloat(holder.itemView, "translationY", if (goesDown == true) 300f else -300f, 0f)
        val animatorRotation = ObjectAnimator.ofFloat(holder.itemView, "rotationX", if (goesDown == true) -90f else 90f, 0f)
